---
name: capture
description: "Capture et organise des déversements d'idées chaotiques en un système structuré et actionnable sans aucune perte d'information. À utiliser dès que l'utilisateur dit 'capture ça', 'brain dump', 'je vais te déverser des idées', 'j'ai plein de pensées', 'voilà tout ce qui me passe par la tête', 'dump d'idées', 'je dois me vider la tête', 'j'ai besoin d'organiser mes pensées', 'voilà ce que je pense', ou toute variation où quelqu'un décharge un flux désordonné d'idées, tâches, réflexions et plans en voulant les rendre cohérents. Se déclenche aussi lorsque l'utilisateur colle ou dicte un long bloc non structuré d'idées mélangées — même sans la formulation exacte — l'intention est la même. Conçu pour une mise en action rapide : pas de saisie préalable. La sortie comprend quatre sections (Projets/Idées, Tâches, Connexions, Comment je peux aider) se terminant par une question directive. Pose au maximum une question de clarification en cours d'organisation lorsqu'un élément est genuinement ambigu entre tâche et projet."
license: MIT
metadata:
