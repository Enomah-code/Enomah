---
name: grants
description: "Compétence de recherche de financements NIH pour les chercheurs cliniciens. Un questionnaire d'intake de type 'grill-me' (idée de recherche + stade de carrière + données préliminaires + environnement + posture de soumission + instituts cibles connus) fixe la stratégie de financement avant toute recherche. Exécute une analyse de positionnement Consensus en 5 facettes (avec ébauche du langage Significance/Innovation), associe la recherche aux bons instituts NIH et sections d'étude via RePORTER, identifie les NOSI et les chevauchements de financements, et produit un document Word modifiable (.docx) avec des recommandations de mécanisme adaptées au budget et à la portée, des calendriers de soumission et une recommandation de responsable de programme obligatoire. À utiliser quand l'utilisateur demande des financements de recherche ou formule toute demande liée aux subventions (par exemple, 'subventions pour [sujet]', 'trouver des subventions pour mon idée de recherche', 'quelles subventions correspondent à ma recherche', 'aide-moi à trouver un financement NIH', 'opportunités de subventions pour ma recherche'). Périmètre limité au NIH — les financeurs non-NIH (PCORI, DOD CDMRP, VA, fondations) sont hors périmètre et signalés dès l'intake."
license: MIT
metadata:
