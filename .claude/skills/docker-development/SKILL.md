...
description: "Skill et plugin d'agent pour le développement Docker et les conteneurs : optimisation de Dockerfile, orchestration docker-compose, builds multi-étapes et durcissement de la sécurité des conteneurs. À utiliser lorsque : l'utilisateur souhaite optimiser un Dockerfile, créer ou améliorer des configurations docker-compose, implémenter des builds multi-étapes, auditer la sécurité des conteneurs, réduire la taille des images ou suivre les bonnes pratiques de conteneurisation. Couvre les performances de build, la mise en cache des couches, la gestion des secrets et les modèles de conteneurs prêts pour la production."
license: MIT
metadata:
  version: 1.0.0
