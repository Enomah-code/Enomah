...
description: "Compétence d'agent et plugin de développement de charts Helm pour Claude Code, Codex, Gemini CLI, Cursor, OpenClaw — scaffolding de charts, conception de valeurs, patterns de templates, gestion des dépendances, renforcement de la sécurité et tests de charts. Utiliser lorsque : l'utilisateur souhaite créer ou améliorer des charts Helm, concevoir des fichiers values.yaml, implémenter des helpers de templates, auditer la sécurité des charts (RBAC, politiques réseau, sécurité des pods), gérer des sous-charts, ou exécuter helm lint/test."
license: MIT
metadata:
  version: 1.0.0
