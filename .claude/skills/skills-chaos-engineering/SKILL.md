...
description: "À utiliser lors de la planification, de l'exécution ou de l'analyse d'expériences de chaos engineering. Se déclenche sur « chaos experiment », « fault injection », « gameday », « resilience test », « blast radius », « steady state », « abort criteria », « Chaos Toolkit », « Chaos Mesh », « Litmus », « Gremlin », « AWS FIS », ou toute question d'injection délibérée de défaillance. Fournit un concepteur d'expériences, un calculateur de rayon d'impact et un générateur de post-mortem (tous en Python stdlib), 4 références sur les principes du chaos + conception d'expériences + taxonomie d'attaques + paysage outillage, et une commande slash /chaos-experiment. Se compose avec feature-flags-architect (kill switches comme déclencheurs d'abandon) et kubernetes-operator (cibles de chaos courantes)."
context: fork
version: 2.9.0
