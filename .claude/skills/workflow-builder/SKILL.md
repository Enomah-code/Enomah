...
description: "Conçoit et rédige des scripts de workflow multi-agents déterministes (fichiers .js dans .claude/workflows/) pour l'outil Workflow de Claude Code. À utiliser quand un utilisateur souhaite créer, rédiger, générer ou exécuter un workflow Claude Code personnalisé, orchestrer des sous-agents (fan-out, pipeline, boucle, panel de juges), ou automatiser une tâche répétable en plusieurs étapes sur des agents à contexte frais."
license: MIT
metadata:
