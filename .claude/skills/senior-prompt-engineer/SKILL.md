...
description: "À utiliser quand l'utilisateur demande à optimiser des prompts, concevoir des modèles de prompts, évaluer des sorties LLM avec un jeu d'évaluation, mesurer la qualité de récupération RAG, valider des configurations d'agents ou d'outils, analyser l'utilisation de tokens, ou concevoir des contrats de sortie structurée. Couvre l'itération de prompts pilotée par les evals, les métriques RAG (pertinence, fidélité, couverture), la validation de workflows d'agents et la budgétisation tokens/coûts — tout agnostique au modèle, avec trois outils Python stdlib."
---
