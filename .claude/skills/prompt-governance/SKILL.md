...
description: "À utiliser pour la gestion de prompts en production à grande échelle : versionnage de prompts, exécution de tests A/B sur les prompts, construction de registres de prompts, prévention des régressions de prompts, ou création de pipelines d'évaluation pour les fonctionnalités IA en production. Déclencheurs : 'gérer les prompts en production', 'versionnage de prompts', 'régression de prompts', 'test A/B de prompts', 'registre de prompts', 'pipeline d'évaluation'. PAS pour rédiger ou améliorer des prompts individuels (utiliser senior-prompt-engineer). PAS pour la conception de pipeline RAG (utiliser rag-architect). PAS pour la réduction des coûts LLM (utiliser llm-cost-optimizer)."
---
