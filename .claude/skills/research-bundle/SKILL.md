---
name: research
description: "Point d'entrée par défaut pour toute demande de recherche — un routeur hybride qui classifie la question de façon déterministe et délègue soit à un skill de recherche spécialisé (pulse pour les tendances/sentiment, grants pour les financements NIH, litreview pour la littérature académique, syllabus pour les lectures de cours, patent pour l'antériorité + le paysage PI, dossier pour la recherche sur une entité), soit exécute son propre workflow de secours plan-décomposition-recherche-multi-sources-synthèse-citation lorsqu'aucun spécialiste ne correspond. Expose toujours la décision de routage pour que l'utilisateur puisse la remplacer. À utiliser lorsque l'utilisateur formule une demande de recherche qui ne correspond pas de façon évidente à un skill spécialisé plus précis (ex. : « recherche [sujet] », « examine [sujet] », « qu'est-ce qu'on sait sur [sujet] », « enquête sur [sujet] », « trouve des informations sur [sujet] », « fais des recherches sur [sujet] », « j'ai besoin de comprendre [sujet] »). La sortie est un briefing Markdown (par défaut) ou un document .docx (sur demande) avec citations complètes et journal d'audit."
---
