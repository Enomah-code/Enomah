...
description: "Transforme les prompts marketing en actifs de production testés et versionnés : évaluation A/B de prompts sur des cas de test structurés, historique immuable des versions de prompts avec diffs, templates de prompts marketing prêts à l'emploi (textes publicitaires, campagnes email, publications sur les réseaux sociaux, pages d'atterrissage, méta SEO), et un guide de gouvernance LLM pour les équipes marketing (discipline sur les affirmations, règles de divulgation, points de contrôle humain). À utiliser lorsqu'une équipe marketing s'appuie sur du contenu généré par IA et a besoin que la qualité des prompts soit mesurable et sécurisée — ou lorsque l'utilisateur mentionne 'ingénierie de prompts', 'améliorer mes prompts', 'templates de prompts', 'versionnage de prompts', 'flux de travail de contenu IA' ou 'gouvernance IA pour le marketing'."
license: MIT
metadata:
  version: 1.0.0
