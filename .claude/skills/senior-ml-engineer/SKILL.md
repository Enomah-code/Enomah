...
description: "Compétence d'ingénierie ML pour la mise en production de modèles, la construction de pipelines MLOps et l'intégration de LLMs. Couvre le déploiement de modèles, les feature stores, la surveillance de dérive, les systèmes RAG et l'optimisation des coûts. À utiliser quand l'utilisateur parle de déployer des modèles ML en production, de mettre en place une infrastructure MLOps (MLflow, Kubeflow, Kubernetes, Docker), de surveiller les performances ou la dérive de modèles, de construire des pipelines RAG, ou d'intégrer des APIs LLM avec logique de retry et contrôle des coûts. Axé sur les préoccupations de production et d'exploitation plutôt que sur la recherche de modèles ou l'entraînement initial."
triggers:
  - MLOps pipeline
  - model deployment
  - feature store
