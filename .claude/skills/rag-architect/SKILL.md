---
name: "rag-architect"
description: "À utiliser lorsque l'utilisateur demande de concevoir un pipeline RAG, de choisir une stratégie de découpage (chunking) ou un modèle d'embedding, de sélectionner une base de données vectorielle, ou d'évaluer la qualité de la récupération (précision@k, rappel@k, NDCG). Exemples : 'concevoir un système RAG pour notre documentation', 'quelle taille de chunk utiliser pour ce corpus', 'évaluer mon moteur de récupération par rapport à la vérité terrain'. PAS pour l'optimisation générale des coûts LLM (utiliser llm-cost-optimizer) ou les boucles d'agents sur la récupération (utiliser agenthub)."
---
