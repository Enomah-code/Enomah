---
name: "vpe-advisor"
description: "Conseil VP Engineering pour startups : débit de livraison (métriques DORA 4 + identification des goulots d'étranglement), entonnoir de recrutement ingénierie (sourcing → présélection → entretien → conversion offre + délai de recrutement + écart pipeline), structure d'équipe ingénierie (conception squad/tribu/chapitre + seuils de déclenchement tech-lead manager), et discipline de production (astreinte, cadence de déploiement, culture de post-mortem). À utiliser quand la vélocité de sprint chute, que le recrutement ingénierie est défaillant, que la structure d'équipe est floue, ou pour décider quand ajouter un tech-lead manager. PAS une compétence CTO (qui détient l'architecture) — le VPE détient les opérations de livraison et la façon dont l'équipe shippe."
license: MIT
metadata:
  version: 1.0.0
