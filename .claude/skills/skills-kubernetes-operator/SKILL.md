...
description: "À utiliser lors de la construction d'un Kubernetes Operator — contrôleurs personnalisés qui réconcilent l'état des CRD. Se déclenche sur « build an operator », « CRD design », « reconcile loop », « controller-runtime », « kubebuilder », « operator-sdk », « metacontroller », « KOPF », « operator capability levels », ou « custom resource ». Fournit un validateur de CRD, un linter de boucle de réconciliation et un auditeur de capacités OperatorHub (tous en Python stdlib), 4 références sur le patron Operator + conception de CRD + patrons de réconciliation + paysage outillage, et une commande slash /operator-audit. PAS un skill k8s générique — spécifiquement le patron Operator."
context: fork
version: 2.9.0
