...
description: "Conseil de Chief Customer Officer pour startups : décomposition de la rétention (rétention brute vs honnêteté du NRR, taxonomie des causes racines du churn), stratégie de segmentation client (investissement différentiel par palier + scoring d'adéquation ICP), modèle de couverture de l'équipe CS (seuils CSM mutualisés vs nominatifs + calcul de ratios), et évolution organisationnelle de l'équipe CS (distinctions CS vs Support vs AM). À utiliser lors de la conception d'une stratégie de rétention, de la segmentation des clients pour un investissement différentiel, du dimensionnement de l'équipe CS, ou de la séquence des recrutements CS. Stratégique uniquement — ne duplique pas les skills tactiques d'ingénierie/croissance commerciale."
license: MIT
metadata:
  version: 1.0.0
