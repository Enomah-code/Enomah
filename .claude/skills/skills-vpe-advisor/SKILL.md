---
name: "vpe-advisor"
description: "Conseil VP Engineering pour startups : débit de livraison (métriques DORA 4 + identification des goulots d'étranglement), entonnoir de recrutement ingénierie (sourcing → présélection → entretien → conversion offre + délai de recrutement + écarts pipeline), structure d'équipe ingénierie (conception squad/tribu/chapitre + seuils de déclenchement tech-lead manager), et discipline de production (astreinte, cadence de déploiement, culture post-mortem). À utiliser quand la vélocité de sprint diminue, le recrutement ingénierie est défaillant, la structure d'équipe est floue, ou pour décider quand ajouter un tech-lead manager. N'est PAS un skill CTO (qui détient l'architecture) — le VPE pilote les opérations de livraison et la façon dont l'équipe livre."
license: MIT
metadata:
  version: 1.0.0
