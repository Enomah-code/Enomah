...
description: "À utiliser lors de la construction d'un Opérateur Kubernetes — des contrôleurs personnalisés qui réconcilient l'état des CRD. Se déclenche sur « construire un opérateur », « conception de CRD », « boucle de réconciliation », « controller-runtime », « kubebuilder », « operator-sdk », « metacontroller », « KOPF », « niveaux de capacité d'opérateur » ou « ressource personnalisée ». Fournit un validateur de CRD, un linter de boucle de réconciliation et un auditeur de capacités OperatorHub (tout en Python stdlib), 4 références sur le pattern Opérateur, la conception de CRD, les patterns de réconciliation et le paysage des outils, ainsi qu'une commande slash /operator-audit. N'est PAS un skill Kubernetes générique — concerne spécifiquement le pattern Opérateur."
context: fork
version: 2.9.0
