...
description: "Configurer, auditer et déboguer l'implémentation du suivi analytique — GA4, Google Tag Manager, taxonomie d'événements, suivi des conversions et qualité des données. À utiliser lors de la construction d'un plan de suivi from scratch, de l'audit d'un analytics existant pour détecter des lacunes ou erreurs, du débogage d'événements manquants, ou de la configuration de GTM. Mots-clés déclencheurs : configuration GA4, Google Tag Manager, GTM, suivi d'événements, implémentation analytics, suivi des conversions, plan de suivi, taxonomie d'événements, dimensions personnalisées, suivi UTM, audit analytics, événements manquants, suivi défaillant. PAS pour analyser les données de campagnes marketing — utiliser campaign-analytics pour cela. PAS pour les tableaux de bord BI — utiliser product-analytics pour l'analyse d'événements in-product."
license: MIT
metadata:
  version: 1.0.0
