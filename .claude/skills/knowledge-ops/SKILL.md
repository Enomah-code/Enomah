---
name: knowledge-ops
description: "À utiliser lorsqu'un Responsable des Opérations, un Knowledge Manager ou un TPM interne doit rédiger, valider ou assainir les SOPs et runbooks internes de l'entreprise (traitement des achats, décommissionnement de fournisseurs, cascade de communication en cas d'incident, onboarding des employés) — incluant les vérifications de complétude 5W2H (Qui-Quoi-Quand-Où-Pourquoi-Comment-Combien), la validation des liens croisés et des pages orphelines dans un wiki Notion/Confluence/Obsidian étendu, l'ingestion en base de connaissances et le reporting de qualité, ainsi que la vérification des étapes de runbook (responsable nommé, durée estimée, signal de succès observable, chemin de rollback, contact d'escalade). Associe la méthode 5W2H d'Ishikawa, *The Checklist Manifesto* de Gawande, ISO 9001, ITIL v4 et la discipline des runbooks du Google SRE Workbook avec des outils Python déterministes (stdlib uniquement) qui évaluent la complétude, détectent les anti-patterns et produisent des listes de corrections prioritaires (ex. : « valider ce runbook avant sa mise en rotation », « auditer notre wiki Confluence pour détecter les SOPs obsolètes et orphelines »)."
context: fork
version: 2.8.0
