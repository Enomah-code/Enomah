...
description: "Conseil de Directeur de la Relation Client pour startups : décomposition de la rétention (honnêteté sur la rétention brute vs NRR, taxonomie des causes racines du churn), stratégie de segmentation client (investissement différencié par niveau + scoring d'adéquation ICP), modèle de couverture de l'équipe CS (seuils CS mutualisé vs nominatif + calcul des ratios), et évolution organisationnelle de l'équipe CS (distinctions CS vs Support vs AM). À utiliser pour concevoir une stratégie de rétention, segmenter les clients pour un investissement différencié, dimensionner l'équipe CS ou séquencer les recrutements CS. Stratégique uniquement — ne duplique pas les compétences tactiques engineering/croissance."
license: MIT
metadata:
  version: 1.0.0
