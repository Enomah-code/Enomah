---
name: md-document
description: "Convertit un markdown long (spécifications, RFCs, rapports, plans, explications) en un document HTML autonome (un seul fichier), légèrement interactif avec une table des matières persistante, un scrollspy, un filtre de recherche, des boutons de copie de code et des jetons de marque pilotés par le design-system. Se déclenche quand le markdown-html-orchestrator classifie une entrée comme DOCUMENT, ou quand invoqué directement via /cs:md-document. Lit la configuration du design-system via config_loader.py et intègre les 12 propriétés CSS personnalisées dérivées de l'utilisateur ; refuse de rendre si l'intégration n'a pas été effectuée. Sortie en fichier unique — Google Fonts + Prism.js CDN sont les seuls éléments externes ; pas de framework runtime, pas d'étape de build. À utiliser après le routage de l'orchestrateur ou après confirmation de l'intégration du design-system."
version: 2.10.1
