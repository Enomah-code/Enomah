...
description: "Boîte à outils d'optimisation pour les stores d'applications (ASO) pour rechercher des mots-clés, analyser les classements des concurrents, générer des suggestions de métadonnées et améliorer la visibilité des applications sur l'Apple App Store et Google Play Store. À utiliser quand l'utilisateur pose des questions sur l'ASO, les classements dans les stores, les métadonnées d'applications, les titres et descriptions d'applications, les fiches store, la visibilité des applications ou le marketing d'applications mobiles sur iOS ou Android. Prend en charge la recherche et le score de mots-clés, l'analyse de mots-clés concurrents, l'optimisation des métadonnées, la planification de tests A/B, les listes de vérification de lancement et le suivi des évolutions de classement."
triggers:
  - ASO
