...
description: "Pipeline de production de contenu complet — prend un sujet de la page blanche à la pièce prête à publier. Utiliser quand vous avez besoin d'exécuter du contenu : rédiger un article de blog, un article de fond ou un guide de bout en bout. Déclencheurs : 'rédige un post sur', 'rédige un article', 'crée du contenu pour', 'aide-moi à écrire', 'j'ai besoin d'un article de blog'. PAS pour la stratégie de contenu ou la planification éditoriale (utiliser content-strategy). PAS pour le recyclage de contenu existant (utiliser content-repurposing). PAS pour les seules légendes de réseaux sociaux."
license: MIT
metadata:
  version: 1.0.0
