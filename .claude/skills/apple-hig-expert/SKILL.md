...
description: "Audite et conçoit des interfaces iOS/macOS/watchOS/visionOS conformément aux Human Interface Guidelines d'Apple, incluant le langage de design Liquid Glass (annoncé à la WWDC25, livré avec iOS 26/macOS Tahoe, septembre 2025). À utiliser lors de la revue d'une maquette ou application Apple pour la conformité HIG, la vérification du contraste ou des tailles de zones tactiles, ou la conception d'interfaces Apple natives (ex. : « auditer mon application iOS par rapport aux HIG », « ce texte est-il lisible sur Liquid Glass ? »)."
license: MIT
metadata:
  version: 1.1.0
