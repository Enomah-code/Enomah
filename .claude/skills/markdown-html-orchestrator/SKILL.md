...
description: "À utiliser quand un utilisateur souhaite convertir un fichier markdown de son projet Claude en un document HTML autonome (un seul fichier), légèrement interactif — documents longs (spécifications, plans, RFCs, rapports, explications), revues de code avec diffs et annotations taguées par sévérité, ou présentations. Se déclenche sur \"convertir ce markdown en HTML\", \"en faire un fichier HTML\", \"transformer ceci en document interactif\", \"rendre ce rapport en HTML\", \"compte rendu de PR en HTML\", \"diapositives à partir de ce markdown\". Délègue le contexte vers l'un des trois sous-skills de conversion (md-document, md-review, md-slides) basé sur un classificateur déterministe de type de document, après que l'utilisateur a exécuté l'intégration design-system une fois. Refuse si l'entrée fait moins de 100 lignes (selon Shihipar — le markdown reste supérieur en dessous du seuil) ou si le design-system n'est pas intégré. Distinct du plugin officiel Anthropic Playground (qui est un outil interactif d'ajustement de prompt avec curseurs/boutons/copie-retour de prompt) et de marketing/landing/ (qui est un générateur de page d'atterrissage)."
context: fork
version: 2.10.3
