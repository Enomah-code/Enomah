---
name: inbox-triage
description: "Effectue un tri complet de la boîte de réception en utilisant la base de connaissances créée par la compétence « inbox-setup ». Conception légère en entrée (la plupart des invocations sautent les questions et s'exécutent avec les préférences par défaut de la base de connaissances) ; pose au maximum 2 questions de substitution grill-me lorsque l'invocation est hors cadence normale ou inclut une intention de saut de catégorie. Recherche les e-mails récents, les classe selon la taxonomie de l'utilisateur, recherche les nouveaux expéditeurs, génère des recommandations, rédige des réponses (N'ENVOIE JAMAIS), produit un rapport dans le format préféré de l'utilisateur, et met à jour la base de connaissances avec les apprentissages. Conçu pour s'exécuter selon un calendrier récurrent (1 à 3 fois par jour) ou à la demande. Utiliser lorsque l'utilisateur souhaite que sa boîte de réception soit traitée, sous toute forme (par ex., « trier ma boîte de réception », « tri de la boîte de réception », « vérifier mes e-mails », « exécuter le tri des e-mails », « traiter ma boîte de réception », « quoi de neuf dans mes e-mails », « gérer mes e-mails », « tri des e-mails »). Nécessite que la compétence inbox-setup ait été exécutée au préalable."
license: MIT
metadata:
