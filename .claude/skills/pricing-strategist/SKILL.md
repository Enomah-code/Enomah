...
description: "À utiliser lors de la conception ou de la révision de la tarification produit — sélection d'un modèle de tarification (abonnement par siège, basé sur l'usage, basé sur la valeur, freemium ou hybride), analyse de la sensibilité au prix Van Westendorp sur des données d'enquête WTP, ou conception de niveaux d'offre Bon/Meilleur/Optimal. Recommande un modèle et une fourchette de prix avec ses compromis, jamais un chiffre unique. Pour les responsables Commerciaux, le Marketing Produit et les CMO au moment de la conception tarifaire — pas pour les remises au cas par cas, ni le positionnement de marque."
version: 2.8.0
