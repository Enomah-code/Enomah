...
description: "Automatisation de la conformité RGPD et DSGVO allemand. Analyse les bases de code pour détecter les risques relatifs à la vie privée, génère la documentation AIPD, suit les demandes d'exercice des droits des personnes concernées avec les délais d'un mois prévus à l'Art. 12(3). À utiliser lors d'évaluations de conformité RGPD, d'audits de confidentialité, de planification de la protection des données, de génération d'AIPD ou de gestion des droits des personnes concernées (DSAR) (par exemple, 'vérifier les risques RGPD de ce service', 'suivre l'échéance d'une demande d'accès'). Les déterminations finales de conformité sont renvoyées au DPO ou au conseil juridique."
---
