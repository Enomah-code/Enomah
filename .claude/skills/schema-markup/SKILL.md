---
name: "schema-markup"
description: "Quand l'utilisateur veut implémenter, auditer ou valider des données structurées (balisage schema) sur son site web. À utiliser quand l'utilisateur mentionne 'données structurées', 'schema.org', 'JSON-LD', 'résultats enrichis', 'rich snippets', 'schema markup', 'schéma FAQ', 'schéma Product', 'schéma HowTo' ou 'erreurs de données structurées dans Search Console'. À utiliser aussi quand quelqu'un demande pourquoi son contenu n'affiche pas de résultats enrichis ou souhaite améliorer sa visibilité dans les recherches IA. PAS pour les audits SEO généraux (utiliser seo-audit) ou les problèmes de crawl SEO technique (utiliser site-architecture)."
license: MIT
metadata:
  version: 1.0.0
