...
description: "Compétence agent et plugin Terraform infrastructure-as-code pour Claude Code, Codex, Gemini CLI, Cursor, OpenClaw. Couvre les patterns de conception de modules, les stratégies de gestion d'état, la configuration des providers, le durcissement de sécurité, le policy-as-code avec Sentinel/OPA, et les workflows CI/CD plan/apply. À utiliser quand : l'utilisateur souhaite concevoir des modules Terraform, gérer des backends d'état, auditer la sécurité Terraform, implémenter des déploiements multi-régions ou suivre les bonnes pratiques IaC."
license: MIT
metadata:
  version: 1.0.0
