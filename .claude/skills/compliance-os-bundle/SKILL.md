---
name: "compliance-os"
description: "Compliance OS — méta-orchestrateur qui permet aux équipes de conformité de CONFIGURER les référentiels applicables, CALCULER les chevauchements de contrôles entre référentiels, SIMULER des audits internes, et CONSOLIDER les preuves sur plusieurs référentiels. Quatre décisions : (1) Étant donné un profil d'entreprise, lesquels des 12 référentiels supportés s'appliquent (ISO 27001/13485/42001/14971, EU AI Act, MDR 745, RGPD, SOC 2, FDA QSR, NIST CSF 2.0, NIS2, HIPAA) ? (2) Sur les référentiels sélectionnés, quels contrôles se chevauchent et quelle part de preuves est réutilisable ? (3) Pour un référentiel et un périmètre donnés, que produit un audit simulé réaliste — puisant dans la bibliothèque de 205 scénarios ? (4) Sur les référentiels sélectionnés, quelle est la liste de preuves unifiée avec la carte de réutilisation ? Utiliser lors de la mise en place d'un programme multi-référentiels, de la planification du calendrier d'audit annuel, ou de la préparation à l'étape 1 de certification. NE remplace PAS les skills par référentiel (il les orchestre)."
license: MIT
metadata:
  version: 1.0.0
