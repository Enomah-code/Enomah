---
name: landing
description: "Génère une landing page HTML premium en une seule page avec animations CSS 3D, effets de défilement GSAP et profondeur parallaxe à la souris. Un questionnaire de cadrage forcé (produit + pitch d'ascenseur, registre d'audience, personnalisation de marque, ton) fixe le positionnement avant toute rédaction ou balisage, de sorte que la page reflète le produit réel plutôt qu'un modèle générique. À utiliser chaque fois que l'utilisateur dit « landing pour X », « créer une landing page », « construire une landing page », « faire une landing page pour X », « j'ai besoin d'une page web pour Y », ou fournit des détails sur un produit/service et souhaite un site web soigné. Se déclenche aussi sur « page promotionnelle », « page produit », « one-pager », « présence web », « page de vente ». Produit un fichier HTML autonome (Claude Code) ou un artefact HTML (Claude.ai). Prend en charge la personnalisation des couleurs de marque via des surcharges de propriétés CSS personnalisées."
license: MIT
metadata:
