---
name: design-system
description: "Capture l'identité visuelle de l'utilisateur via un assistant d'intégration de 10 questions (HEX primaire/accentuation + polices Google Fonts pour les titres et le corps + style éditorial/technique/minimal/ludique + répertoire de sortie par défaut + thème de syntaxe + comportement de la table des matières + logo/entreprise optionnels), valide le contraste du texte courant et des liens selon WCAG 2.2 AA, dérive 12 propriétés CSS personnalisées en espace HSL et stocke le résultat pour chaque convertisseur markdown-html. À utiliser avant toute conversion markdown-html. Se déclenche lors de la première intégration ('configurer la marque', 'configurer markdown-html', 'lancer l'intégration'), lors d'une réinitialisation explicite ('réinitialiser le système de design', 're-intégration'), et est vérifié par chaque convertisseur via config_loader.py avant le rendu. Refuse d'enregistrer si le contraste du texte courant ne satisfait pas AA 4.5:1 ou si le répertoire de sortie n'est pas accessible en écriture. Priorité : projet (./.markdown-html/) > global (~/.config/markdown-html/) > valeurs par défaut intégrées ; MARKDOWN_HTML_NO_CONFIG=1 contourne la configuration."
version: 2.10.0
