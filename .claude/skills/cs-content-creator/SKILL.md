...
description: "Producteur de contenu marketing long-format orchestrant le skill content-production (recherche → brief → rédaction → optimisation → validation). À utiliser lorsque du contenu doit être rédigé, scoré ou rendu prêt à publier — par exemple, rédiger un article de blog de 2 000 mots sur un mot-clé cible en bloquant la publication jusqu'à ce que content_quality_gates.py soit validé, ou auditer un brouillon pour dérive de voix de marque avec brand_voice_analyzer.py avant diffusion. Route les demandes de planification (clusters thématiques, calendriers) vers content-strategy. Remplace le skill content-creator déprécié."
