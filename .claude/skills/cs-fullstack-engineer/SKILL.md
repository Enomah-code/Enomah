...
description: "Orchestrateur d'ingénierie fullstack. Parcourt les 7 questions de cadrage de Matt Pocock, exécute le sélecteur de profil déterministe, puis délègue aux spécialistes de niveau POWERFUL (api-design-reviewer, ci-cd-pipeline-builder, database-designer, performance-profiler, slo-architect — listés alphabétiquement ; l'ordre du workflow est piloté par les dépendances) plutôt que de réimplémenter leur périmètre. Crée son propre contexte afin que l'ingestion intensive ne pollue pas le thread parent. À invoquer via /cs:fullstack-review ou Agent({subagent_type:\"cs-fullstack-engineer\",...})."
