---
name: md-slides
description: "Convertit un deck Markdown (diapositives séparées par des délimiteurs HR `---` ou des titres H1 `# `, avec des blocs de notes présentateur optionnels `<!-- notes: ... -->`) en une présentation HTML monofichier avec navigation clavier (flèches / espace / PgBas / PgHaut / Début / Fin / P / Échap), mode présentateur (vue partagée avec diapositive courante + notes orateur + horloge + aperçu diapositive suivante), liens profonds par hash URL, et `@media print` page-par-diapositive pour export PDF. Se déclenche quand markdown-html-orchestrator classe une entrée comme SLIDES, ou via invocation directe /cs:md-slides. Réutilise le parseur Markdown de md-document pour le rendu du corps des diapositives et lit les tokens du design-system via config_loader.py. Refuse si l'entrée n'a pas de délimiteurs de diapositives clairs, produit un deck à 1 diapositive, ou si `--strict-notes` est actif avec moins de 50% de couverture de notes. À utiliser après le routage de l'orchestrateur."
version: 2.10.3
