---
name: md-review
description: "Convertit un compte rendu de PR ou une revue de code en markdown (contenant des blocs ```diff délimités et des légendes taguées par sévérité > [!BLOCKER]/[!MAJOR]/[!MINOR]/[!NIT]) en une revue HTML à 2 colonnes autonome (un seul fichier) — diff unifié à gauche, cartes d'annotation taguées par sévérité à droite, navigation de saut en haut listant chaque constat, pied de page obligatoire avec le nom du relecteur. Se déclenche quand le markdown-html-orchestrator classifie une entrée comme REVIEW, ou quand invoqué directement via /cs:md-review. Refuse sans --reviewer explicite (une revue de code doit nommer un humain), refuse si aucune section diff n'est présente (rediriger vers md-document à la place), et refuse d'encoder la sévérité uniquement par couleur (chaque badge inclut couleur + icône + aria-label conformément à WCAG 1.4.1). À utiliser après le routage de l'orchestrateur."
version: 2.10.2
