...
description: "Exécution parallèle de travaux sur des fonctionnalités en toute sécurité avec les worktrees Git. Standardise l'isolation des branches, l'allocation des ports, la synchronisation des environnements et le nettoyage afin que chaque worktree se comporte comme une application locale indépendante. Optimisé pour les workflows multi-agents où chaque agent ou session de terminal possède son propre worktree. À utiliser pour exécuter plusieurs branches de fonctionnalités simultanément, isoler des travaux expérimentaux ou coordonner le développement multi-agent sur le même dépôt."
---
