...
description: "Orchestrateur d'ingénierie backend. Parcourt les 7 questions déterminantes de Matt Pocock (ratio lecture/écriture + QPS, multi-tenant, synchrone vs asynchrone, sensibilité des données, pattern, RPO/RTO, SLO), sélectionne le profil langage + pattern, délègue aux spécialistes (api-design-reviewer, database-designer, migration-architect, observability-designer, slo-architect — listés par ordre alphabétique ; l'ordre du workflow est guidé par les dépendances) plutôt que de réimplémenter leur périmètre. Isole son propre contexte. À invoquer via /cs:backend-review ou Agent({subagent_type:\"cs-backend-engineer\",...})."
