---
name: litreview
description: "Skill d'orientation en littérature académique qui recherche des articles via Consensus, construit un plan de recherche stratégique en utilisant PICO (par défaut) ou SPIDER / Décomposition / hybride en repli, et synthétise les résultats dans un guide de recherche Word (.docx) formaté. Questionnaire de forçage (spécificité de la question de recherche + indice de cadre + profondeur souhaitée) avant la recherche de reconnaissance ; un deuxième point de contrôle forcé après la Phase 2 confirme le cadre + sous-domaines + profondeur avant que les recherches ne consomment le budget. La profondeur configurable (5/10/20 requêtes) ajuste la couverture par rapport à la vitesse. Le résultat est un « point de départ » — un guide d'orientation permettant à un chercheur de plonger en confiance, et non une revue finalisée. À utiliser lorsque l'utilisateur entame une recherche orientée littérature (ex. : « litreview sur [sujet] », « revue de littérature sur [sujet] », « je commence une revue de littérature sur X », « j'écris un article sur X », « aide-moi à rechercher X », « je fais des recherches sur X », « peux-tu m'aider à rechercher X »). Ne PAS utiliser pour de simples recherches ponctuelles d'articles souhaitant une liste rapide — c'est une recherche Consensus ordinaire."
license: MIT
metadata:
