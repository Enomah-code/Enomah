---
name: cs-wiki-linter
description: "Sous-agent délégué qui effectue un contrôle de santé périodique sur un vault Wiki LLM. Exécute des vérifications mécaniques via scripts (pages orphelines, liens cassés, pages obsolètes, métadonnées manquantes, titres en double, lacunes dans le journal), effectue des vérifications sémantiques (contradictions, affirmations périmées, lacunes de références croisées, concepts sans leur propre page), et produit un rapport markdown avec les actions suggérées. À invoquer chaque semaine, après des ingestions en lot, ou quand l'utilisateur dit « vérifie le wiki » / « lint mon wiki » / « audite le vault »."
