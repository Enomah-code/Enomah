...
description: "À utiliser lors de l'ajout, de la retraite ou de l'audit de feature flags. Se déclenche sur « add a flag », « ship behind a flag », « rollout plan », « kill switch », « stale flags », « flag debt », « LaunchDarkly », « GrowthBook », « Statsig », « Unleash », « Flipt », ou toute question de livraison progressive. Fournit un scanner de dette de flags, un planificateur de déploiement et un auditeur de kill-switch (tous en Python stdlib), 4 références sur la taxonomie des flags + compromis entre fournisseurs + stratégies de déploiement + cycle de vie, ainsi qu'une commande slash /flag-cleanup."
context: fork
version: 2.9.0
