...
description: "Quand l'utilisateur souhaite auditer, repenser ou planifier la structure de son site web, la hiérarchie des URL, la conception de la navigation ou la stratégie de maillage interne. À utiliser quand l'utilisateur mentionne « architecture de site », « structure des URL », « liens internes », « navigation du site », « fil d'Ariane », « clusters thématiques », « pages piliers », « pages orphelines », « structure en silo », « architecture de l'information » ou « réorganisation du site web ». À utiliser aussi quand quelqu'un a des problèmes SEO dont la cause racine est structurelle (pas liée au contenu ou au schéma). PAS pour les décisions de stratégie de contenu sur quoi rédiger (utiliser content-strategy) ni pour le balisage schéma (utiliser schema-markup)."
license: MIT
metadata:
  version: 1.0.0
