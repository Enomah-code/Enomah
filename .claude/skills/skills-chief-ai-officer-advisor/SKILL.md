...
description: "Conseil de Chief AI Officer pour startups : décisions de construction vs achat de modèles (API vs fine-tuning vs en interne), classification des risques IA sous l'AI Act européen + patchwork d'États américains, économie des coûts IA (seuil de rentabilité API vers auto-hébergé), et évolution organisationnelle de l'équipe IA. À utiliser lors du choix entre appel d'API ou fine-tuning, la classification des cas d'usage IA pour le risque réglementaire, le calcul du moment où l'auto-hébergement devient rentable, la séquence des recrutements IA, ou lorsque l'utilisateur mentionne CAIO, stratégie IA, sélection de modèle, modèle fondateur, fine-tuning, EU AI Act, NIST AI RMF, gouvernance IA, risque modèle ou économie IA. Stratégique uniquement — ne duplique pas les skills d'ingénierie IA/ML."
license: MIT
metadata:
  version: 1.0.0
