---
name: dossier
description: "Skill de recherche d'entités à niveau décisionnel — produit un dossier testé sur hypothèse concernant une entreprise, une personne, une organisation à but non lucratif ou une organisation gouvernementale spécifique, et non un profil générique. La collecte forcée oblige l'utilisateur à énoncer son hypothèse en amont (ce qu'il croit déjà et souhaite vérifier ou infirmer) afin que le dossier la teste plutôt que la confirme. Le résultat est un document Word modifiable (.docx) avec le verdict sur l'hypothèse, les faits d'identité, une chronologie d'activité sur 12 mois, les signaux de réseau et de réputation, les signaux d'alerte, les accroches conversationnelles liées aux conclusions spécifiques et le journal d'audit de provenance des sources. Utilise WebSearch + WebFetch + API gratuites (SEC EDGAR, GitHub, ProPublica) comme outils principaux ; des MCPs BYOK optionnels améliorent la couverture. À utiliser lorsque l'utilisateur demande des recherches de contexte, de la diligence raisonnable ou une préparation à une réunion concernant une entité spécifique (par ex., 'prépare-moi pour une réunion avec [personne/entreprise]', 'diligence raisonnable sur [entreprise]'). Respecte les exclusions de sensibilité pour les contextes journalistiques et de vérification personnelle."
license: MIT
metadata:
