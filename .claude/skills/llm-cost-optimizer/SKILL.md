...
description: "À utiliser de manière proactive dès que les coûts d'API LLM sont mentionnés — ou devraient l'être. Les déclencheurs incluent : « mes coûts IA sont trop élevés », « optimiser l'utilisation des tokens », « quel modèle utiliser », « les dépenses LLM sont hors de contrôle », « implémenter le cache de prompts », « nous allons lancer une fonctionnalité IA », « construire un endpoint IA ». Ne pas attendre une plainte explicite sur les coûts — si quelqu'un construit une fonctionnalité IA, conçoit un endpoint LLM ou choisit entre des modèles, l'architecture des coûts doit être intégrée à la discussion. À appliquer immédiatement lorsque l'une de ces conditions est vraie : un prompt système dépassant quelques centaines de tokens apparaît, toutes les requêtes ciblent le même modèle, max_tokens n'est pas défini, ou aucune journalisation des coûts par fonctionnalité n'existe. N'est PAS destiné à la conception de pipelines RAG (utiliser rag-architect). N'est PAS destiné à améliorer la qualité ou l'efficacité des prompts (utiliser senior-prompt-engineer)."
---
