...
description: "Hook de sécurité PreToolUse pour Claude Code détectant les anti-patterns. Intercepte 12 risques de sécurité courants (injection de commandes, XSS, injection SQL, désérialisation non sécurisée, injection dans les workflows GitHub Actions, injection de code via eval/new Function) AVANT que l'opération Edit/Write/MultiEdit ne soit finalisée. La mise en cache de l'état de session évite les avertissements en double pour la même combinaison fichier+règle. Bibliothèque standard uniquement, sans dépendances. À utiliser pour disposer d'un filet de sécurité lors des sessions Claude Code touchant du code sensible (authentification, paiements, traitement des entrées utilisateur, IaC). Désactiver avec ENABLE_SECURITY_REMINDER=0 si vous devez effectuer une opération vérifiée sûre qui déclencherait sinon un pattern. Déclencheurs — 'ajouter un hook de sécurité', 'bloquer du code non sécurisé', 'détecter l'injection de commandes avant écriture', 'prévenir les patterns d'injection SQL', 'hook d'avertissement de sécurité'."
---
