...
description: "À utiliser pour gérer le budget d'un programme ou d'un portefeuille R&D interne — construction d'un budget de programme multi-périodes avec la ventilation F&A (indirects), suivi du taux de consommation et de la visibilité financière par rapport aux jalons de création de valeur, ou affectation des postes de coûts R&D à une décision d'activation ou de comptabilisation en charges. Chaque sortie budgétaire expose son bloc d'hypothèses ; la décision activation/charges est une aide à la décision uniquement et est transmise à un responsable financier nommé — elle ne comptabilise jamais une écriture ni ne statue sur le traitement comptable. Distinct de finance/analyse-financière (DCF d'entreprise, clôture, valorisation) et de recherche/grants (découverte de financements — ce skill gère l'argent déjà obtenu)."
version: 2.9.0
