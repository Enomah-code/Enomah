...
description: "Compétence de data scientist senior de niveau mondial, spécialisée en modélisation statistique, conception d'expériences, inférence causale et analytique prédictive. Couvre les tests A/B (calcul de taille d'échantillon, tests z à deux proportions, correction de Bonferroni), les différences-en-différences, les pipelines d'ingénierie de features (Scikit-learn, XGBoost), l'évaluation de modèles par validation croisée (AUC-ROC, AUC-PR, SHAP) et le suivi d'expériences MLflow — en utilisant Python (NumPy, Pandas, Scikit-learn), R et SQL. À utiliser pour concevoir ou analyser des expériences contrôlées, construire et évaluer des modèles de classification ou de régression, effectuer des analyses causales sur des données observationnelles, ingénier des features pour des jeux de données tabulaires structurés, ou traduire des résultats statistiques en décisions métier fondées sur les données."
---
