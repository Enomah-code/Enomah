...
description: "Surveille la santé des clients, prédit le risque de churn, et identifie les opportunités d'expansion grâce à des modèles de scoring pondérés pour le succès client SaaS. À utiliser pour analyser des comptes clients, examiner les métriques de rétention, scorer les clients à risque, ou quand l'utilisateur mentionne churn, scores de santé client, opportunités d'upsell, revenus d'expansion, analyse de rétention, ou analytique client. Exécute trois outils CLI Python pour produire des scores de santé déterministes, des niveaux de risque de churn, et des recommandations d'expansion priorisées pour les segments Enterprise, Mid-Market et PME."
license: MIT
metadata:
  version: 1.0.0
