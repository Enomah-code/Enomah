---
name: "agent-designer"
description: "À utiliser quand l'utilisateur souhaite concevoir un système multi-agents, choisir un schéma d'orchestration (superviseur/essaim/pipeline), générer des schémas d'outils pour des agents, ou évaluer des journaux d'exécution d'agents pour identifier les coûts, latences et goulots d'étranglement. Exemples : « concevoir une architecture d'agents pour l'automatisation de la recherche », « générer des schémas d'outils Anthropic à partir de ces descriptions d'outils », « analyser ces journaux d'exécution d'agents pour détecter les goulots d'étranglement ». PAS pour les fichiers de workflow Claude Code (utiliser workflow-builder) ni pour la conception de prompts mono-agent (utiliser agent-workflow-designer)."
---
