---
name: inbox-setup
description: "Compétence de configuration unique qui construit une base de connaissances personnalisée pour le tri de la boîte de réception via une interview interactive. Interroge l'utilisateur sur ses habitudes de messagerie, le contexte professionnel, le style de réponse et les priorités selon la discipline grill-me (une question à la fois, format forcé si possible, ordonnées par dépendances, chaque question explique pourquoi elle est posée), puis génère les fichiers de base de connaissances qui alimentent la compétence compagnon « inbox-triage ». Exécuter une fois avant d'utiliser inbox-triage pour la première fois. Ré-exécuter lorsque le contexte commercial, la tarification ou les priorités changent significativement. Déclencheurs : « configurer ma boîte de réception », « configurer le tri de la boîte de réception », « configurer mon système de messagerie », « configurer le tri des e-mails », « construire ma base de connaissances e-mail », « initialiser la gestion des e-mails », ou toute variation où quelqu'un souhaite mettre en place le système de tri des e-mails pour la première fois."
license: MIT
metadata:
