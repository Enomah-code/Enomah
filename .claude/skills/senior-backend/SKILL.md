---
name: "senior-backend"
description: "Conçoit et implémente des systèmes backend incluant des API REST, des microservices, des architectures de base de données, des flux d'authentification et le durcissement de la sécurité. À utiliser quand l'utilisateur demande de 'concevoir des API REST', 'd'optimiser des requêtes de base de données', 'd'implémenter l'authentification', 'construire des microservices', 'revoir du code backend', 'configurer GraphQL', 'gérer des migrations de base de données' ou 'tester la charge des API'. Couvre le développement Node.js/Express/Fastify, l'optimisation PostgreSQL, la sécurité des API et les patterns d'architecture backend."
---
