---
name: patent
description: "Skill de recherche d'antériorité et d'intelligence du paysage brevets — pas d'aide générique sur les brevets. Valide l'un des cinq sous-cas d'usage via une prise en charge forcée (recherche de nouveauté / liberté d'exploitation / paysage concurrentiel / diligence raisonnable d'acquisition / antériorité en litige) avant toute recherche. Interroge Google Patents, Espacenet, USPTO et optionnellement Lens.org pour les signaux du graphe de citations. Le résultat est un document Word éditable (.docx) avec verdict, art antérieur le plus proche classé (texte de revendication extrait), paysage par classe CPC, résultats par famille de brevets, couverture géographique, alertes FTO le cas échéant, recommandations stratégiques et journal d'audit complet. Utiliser quand l'utilisateur demande une recherche ou une analyse de brevets (ex. : \"recherche d'antériorité pour [invention]\", \"analyse de liberté d'exploitation pour [produit]\"). Produit un signal de recherche, pas un conseil juridique — recommande toujours de consulter un conseil en brevets avant tout dépôt ou décision de licence. Les questions relatives aux marques, droits d'auteur et secrets commerciaux sont hors périmètre."
license: MIT
metadata:
