...
description: "À utiliser lors de l'ajout, la suppression ou l'audit de feature flags. Se déclenche sur « ajouter un flag », « livrer derrière un flag », « plan de déploiement progressif », « kill switch », « flags obsolètes », « dette de flags », « LaunchDarkly », « GrowthBook », « Statsig », « Unleash », « Flipt », ou toute question de livraison progressive. Fournit un scanner de dette de flags, un planificateur de rollout et un auditeur de kill-switch (tous en Python stdlib), 4 références sur la taxonomie des flags, les compromis entre fournisseurs, les stratégies de rollout et le cycle de vie, ainsi qu'une commande slash /flag-cleanup."
context: fork
version: 2.9.0
