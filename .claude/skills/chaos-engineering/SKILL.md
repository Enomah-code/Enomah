...
description: "À utiliser lors de la planification, l'exécution ou le retour d'expérience d'expériences de chaos engineering. Se déclenche sur 'expérience chaos', 'injection de fautes', 'gameday', 'test de résilience', 'rayon de souffle', 'état stable', 'critères d'abandon', 'Chaos Toolkit', 'Chaos Mesh', 'Litmus', 'Gremlin', 'AWS FIS', ou toute question d'injection délibérée de pannes. Fournit un concepteur d'expériences, un calculateur de rayon de souffle et un générateur de postmortem (tous en Python stdlib), 4 références sur les principes du chaos + conception d'expériences + taxonomie des attaques + panorama des outils, et une commande slash /chaos-experiment. Se compose avec feature-flags-architect (kill switches comme déclencheurs d'abandon) et kubernetes-operator (cibles communes du chaos)."
context: fork
version: 2.9.0
