...
description: "Boucle d'expérimentation autonome qui optimise n'importe quel fichier selon une métrique mesurable. Inspiré de l'autoresearch de Karpathy. L'agent modifie un fichier cible, exécute une évaluation fixe, conserve les améliorations (git commit), rejette les échecs (git reset) et boucle indéfiniment. À utiliser quand : l'utilisateur souhaite optimiser la vitesse du code, réduire la taille du bundle/image, améliorer le taux de réussite des tests, optimiser des prompts, améliorer la qualité du contenu (titres, textes, CTR), ou exécuter toute boucle d'amélioration mesurable. Prérequis : un fichier cible, une commande d'évaluation qui produit une métrique, et un dépôt git."
license: MIT
metadata:
  version: 2.0.0
