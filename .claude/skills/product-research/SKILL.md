...
description: "À utiliser lors de la planification et de la synthèse de la recherche produit/utilisateur en tant que discipline méthode-et-référentiel — sélection de la bonne méthode selon l'objectif (entretiens génératifs vs test d'utilisabilité vs test de concept vs validation), calcul de la saturation/taille d'échantillon basée sur la méthode avec un niveau de confiance explicite, ou synthèse des observations codées en insights tout en signalant les anecdotes à source unique. Ne fabrique jamais d'insight utilisateur ; un insight requiert une récurrence chez plusieurs participants indépendants. Distinct de product-team/ux-researcher-designer (artefacts persona/parcours), product-discovery (planification de sprint de discovery) et experiment-designer (A/B live) — il s'agit de la couche méthode de recherche-ops + référentiel d'insights."
version: 2.9.0
