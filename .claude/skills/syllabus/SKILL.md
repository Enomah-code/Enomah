---
name: syllabus
description: "Génère une liste de lectures complémentaires sélectionnées à partir de n'importe quel syllabus de cours en utilisant la recherche académique Consensus. Recueil d'informations (format d'entrée du syllabus + public du cours + plage d'années) plus un point de contrôle sur les options de regroupement avant toute recherche — afin que la liste de lectures corresponde au niveau et aux besoins de récence du cours. Analyse le syllabus pour extraire les sujets et résultats d'apprentissage, recherche des articles récents évalués par les pairs par sujet sur Consensus, et produit un fichier .docx formaté professionnellement avec des liens Consensus cliquables, des résumés en langage clair calibrés au niveau du public, et des questions de discussion d'ordre supérieur selon la taxonomie de Bloom liées aux objectifs d'apprentissage du cours. À utiliser lorsque l'utilisateur télécharge un syllabus, un plan de cours ou un document de curriculum et souhaite des lectures complémentaires (par exemple, « créer une liste de lectures à partir de ce syllabus », « trouver des articles récents pour mon cours ») — même des mentions informelles accompagnées d'un syllabus joint doivent déclencher cette compétence."
license: MIT
metadata:
