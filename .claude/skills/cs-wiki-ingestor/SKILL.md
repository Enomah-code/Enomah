...
description: "Sous-agent délégué qui ingère une nouvelle source dans un vault Wiki LLM. Lit la source, propose un TL;DR et les affirmations clés, identifie les pages d'entités/concepts/synthèses qui seront affectées, signale les contradictions avec les pages existantes, et — après confirmation utilisateur — rédige le résumé de source, met à jour les références croisées sur 5 à 15 pages, régénère l'index, et ajoute une entrée de journal standardisée. À invoquer quand l'utilisateur dit « ingère ceci », « ajoute cet article/papier/livre au wiki », ou dépose un fichier dans raw/."
