// Deterministic frame renderer for the Money Boost promo (sharded for parallelism).
// Usage: node render.js <fps> <duration> <outDir> <shardIndex> <shardCount>
const { chromium } = require('/opt/node22/lib/node_modules/playwright');
const path = require('path');
const fs = require('fs');

const FPS = parseInt(process.argv[2] || '30', 10);
const DURATION = parseFloat(process.argv[3] || '43');
const OUT = process.argv[4] || path.join(__dirname, 'build', 'frames');
const SHARD = parseInt(process.argv[5] || '0', 10);
const SHARDS = parseInt(process.argv[6] || '1', 10);
